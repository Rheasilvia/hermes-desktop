...
class TestSessionTitleIndexPolicy:
    @staticmethod
    def _seed_database(tmp_path, *, duplicate_titles, legacy_unique_index=False):
        ...
        if duplicate_titles:
            session_db.set_session_title("older", "shared-title")
            session_db.set_session_title(
                "newer", "shared-title", enforce_unique=False
            )
        session_db.close()
    ...
    def test_duplicate_titles_are_preserved_without_deleting_sessions(self, tmp_path):
        db_path = self._seed_database(tmp_path, duplicate_titles=True)
        ...
        try:
            ...
            assert indexes["idx_sessions_title"] == 0
        finally:
            reopened.close()

    def test_api_policy_rejects_duplicates_unless_surface_opts_out(self, tmp_path):
        db_path = self._seed_database(tmp_path, duplicate_titles=True)
        ...
        try:
            ...
            assert reopened.set_session_title(
                "future", "shared-title", enforce_unique=False
            )
        finally:
            reopened.close()

    def test_legacy_unique_index_is_replaced_without_clearing_titles(self, tmp_path):
        db_path = self._seed_database(
            tmp_path, duplicate_titles=False, legacy_unique_index=True
        )
