        ...
        if row is None:
            ...
            if current_version < 11:
                ...
                if fts5_available:
                    ...
                    fts_migrations_complete = False
            # v12 (messages.active backfill) and v15 (drop unique title index)
            # were version-gated migrations on this fork. Both are now subsumed
            # by stronger unconditional guards that run on every startup:
            #   - active=1 repair: see the "Heal NULL active rows" block above
            #     (upstream fix for #51646 — runs regardless of schema_version,
            #      whereas the old `current_version < 12` gate never re-ran for
            #      already-v12+ databases).
            #   - title index: the post-migration block below unconditionally
            #     drops idx_sessions_title_unique and creates a non-unique
            #     idx_sessions_title, so titles can repeat (the desktop surface
            #     relies on this via set_session_title(..., enforce_unique=False)).
