...
class SessionDB:
    ...
    def _init_schema(self):
        ...
        # Title uniqueness is an API policy rather than a storage constraint:
        # CLI/TUI/gateway writes use enforce_unique=True, while the Tauri
        # desktop treats titles as display labels and opts out explicitly.
        # Drop legacy/upstream unique indexes on every startup so both policies
        # can safely share one state database.
    ...
    def set_session_title(
        ...
        Raises ValueError if the title fails validation, or when
        ``enforce_unique`` is true and another non-ancestor session already
        uses it. The Tauri desktop passes ``False`` because titles are display
        labels there; other surfaces retain the unique-alias behavior.
