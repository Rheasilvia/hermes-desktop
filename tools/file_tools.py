...
def _authoritative_workspace_root(task_id: str = "default") -> str | None:
    ...
    Resolution order:
      1. The session's own cwd record — updated after completed terminal
         commands and keyed by raw session id, so one session's ``cd`` cannot
         leak into another session.
      2. The per-thread ``terminal_cwd`` ContextVar — how the Tauri daemon
      ...
      3. A session-specific registered override (TUI/ACP sessions via
         ...
         session resolves its own worktree from the first tool call.
      4. A sentinel-free, absolute ``$TERMINAL_CWD`` env var (TUI/CLI/cron).
    ...
    try:
        ...
        recorded = _sentinel_free_abs_cwd(get_session_cwd(task_id))
    except Exception:
        recorded = None
    if recorded:
        return recorded

    # The Tauri sidecar binds each turn's workspace through a ContextVar rather
    # than terminal task registration. Keep it ahead of process-wide fallbacks.
    try:
        from tools.terminal_cwd import get_context_cwd

        context_cwd = _sentinel_free_abs_cwd(get_context_cwd())
    except Exception:
        context_cwd = None
    if context_cwd:
        return context_cwd
...
def _check_workspace_boundary(
    ...
    # `resolved` may be a PurePosixPath when the active terminal backend is
    # container-based (_uses_container_paths / _resolve_path_for_task) — it
    # names a location inside the sandbox, not on the host. PurePosixPath has
    # no .resolve(), so validate_within_dir (which calls path.resolve()) would
    # raise AttributeError and abort the read/write/search entirely. Coerce to
    # a concrete Path for this membership check only — it does not affect the
    # path used for the actual file I/O — so a container path simply falls
    # through to the same approval-gated "outside workspace" flow used for
    # any other out-of-workspace path, instead of crashing.
    error = validate_within_dir(Path(resolved), Path(workspace_root))
    if error is None:
