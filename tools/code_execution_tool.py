...
def _resolve_child_cwd(mode: str, staging_dir: str, task_id: str = "") -> str:
    ...
    # 2. The Tauri desktop binds each turn's workspace through a ContextVar.
    try:
        from tools.terminal_cwd import get_context_cwd

        context_cwd = (get_context_cwd() or "").strip()
    except Exception:
        context_cwd = ""
    if context_cwd:
        expanded = os.path.expanduser(context_cwd)
        if os.path.isdir(expanded):
            return expanded

    if task_id:
        # 3. Registered workspace override (session.cwd.set → gateway/TUI/ACP).
        try:
