...
def _clean_generated_title(content: str) -> Optional[str]:
    """Normalize a model response into a bounded, single-line title."""
    from agent.agent_runtime_helpers import strip_think_blocks

    title = strip_think_blocks(None, content or "").strip().strip('"\'')
    ...
    if agent:
        try:
            title = _clean_generated_title(
                _call_with_agent(agent, messages, model=title_model) or ""
            )
            if title:
                return title
...
def _persist_session_title(session_db, session_id, title):
    ...
    title would collide with another session under its uniqueness policy. Rather
