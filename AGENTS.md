...
### Hermes Studio (`apps/hermes-studio/`)

Hermes Studio is the Electron + SolidJS application in this fork. It has a
Studio-owned Python sidecar and a frozen typed preload API. Electron main owns
native capabilities and sidecar lifecycle; the renderer never receives Node,
raw Electron APIs, generic IPC, or packaged backend credentials.

Studio is independent of the React application under `apps/desktop/`. Treat
that application as reference material only; do not import its renderer, state,
or runtime modules into Studio.

Scoped contributor rules live in `apps/hermes-studio/AGENTS.md`. Architecture
authority lives in `apps/hermes-studio/docs/ARCHITECTURE.md`, the native bridge
contract in `apps/hermes-studio/docs/NATIVE_BRIDGE.md`, and the host decision in
`apps/hermes-studio/docs/decisions/ADR-001-electron-shell.md`. Run Studio
commands from `apps/hermes-studio/`: `npm run dev`, `npm run build`,
`npm run pack`, `npm run dist`, `npm run typecheck`, `npm run lint`,
`npm run test`, `npm run test:e2e`, and `npm run docs:check`.
