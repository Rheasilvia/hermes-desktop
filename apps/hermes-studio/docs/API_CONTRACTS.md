...
Hermes Studio preserves the `/desktop/api` REST and SSE routes and their
payloads. Electron main owns how the local process is bound, discovered,
authenticated, and supervised.
...
Other localhost ports, arbitrary HTTP origins, and former host origins are not
accepted. Requests continue to use `Authorization: Bearer <api-token>`.
