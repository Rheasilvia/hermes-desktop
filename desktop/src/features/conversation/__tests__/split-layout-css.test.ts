...
describe('conversation split layout CSS', () => {
  ...
  it('anchors the right tools pane as a split-grid sibling with a top-to-bottom divider', () => {
    ...
    expect(toolTabActiveRule).toContain('background: var(--color-surface)');
    expect(toolTabActiveRule).toContain('inset 1px 0 0 var(--color-border-light)');
  ...
  it('virtualizes heavy diff content for live Review resizing', () => {
    ...
    const reviewBodySplitRailRule = ruleBody(css, '.diffReviewBodySplit .diffFileRail');
    const splitHandleRule = ruleBody(css, '.reviewSplitHandle');
    const fileRailRule = ruleBody(css, '.diffFileRail');
    const fileNavigatorRule = ruleBody(css, '.diffFileNavigator');
    const fileListRule = ruleBody(css, '.diffFileList');
    const fileListSurfaceRule = ruleBody(css, '.diffFileListSurface');
    const fileRowRule = ruleBody(css, '.diffFileRow');
    const fileRowSelectRule = ruleBody(css, '.diffFileRowSelect');
    const fileRowActionsRule = ruleBody(css, '.diffFileRowActions');
    const drawerRule = ruleBody(css, '.diffFileDrawer');
    const headerFilesButtonRule = ruleBody(css, '.diffHeaderFilesButton');
    const headerFilesLabelRule = ruleBody(css, '.diffHeaderFilesLabel');
    ...
    expect(headerRule).toContain('gap: 6px');
    ...
    expect(reviewBodySplitRule).toContain('grid-template-columns: minmax(0, 1fr) var(--review-split-handle-width, 8px) var(--review-file-rail-width, 304px)');
    expect(reviewBodySplitRailRule).toContain('display: flex');
    expect(reviewBodySplitRailRule).toContain('grid-column: 3');
    expect(splitHandleRule).toContain('grid-column: 2');
    expect(splitHandleRule).toContain('cursor: col-resize');
    expect(splitHandleRule).toContain('touch-action: none');
    expect(fileRailRule).toContain('display: none');
    expect(fileRailRule).toContain('border-left: 1px solid var(--color-border-light)');
    expect(fileNavigatorRule).toContain('width: 100%');
    expect(fileNavigatorRule).toContain('flex: 1 1 auto');
    expect(fileListRule).toContain('overflow: auto');
    expect(fileListRule).toContain('contain: layout style paint');
    expect(fileListSurfaceRule).toContain('width: 100%');
    expect(fileRowRule).toContain('position: absolute');
    expect(fileRowRule).toContain('display: block');
    expect(fileRowSelectRule).toContain('grid-template-columns: 8px minmax(0, 1fr) auto');
    expect(fileRowSelectRule).toContain('var(--file-depth-offset, 0px)');
    expect(fileRowActionsRule).toContain('position: absolute');
    expect(fileRowActionsRule).toContain('right: var(--space-2)');
    expect(fileRowActionsRule).toContain('pointer-events: none');
    expect(css).not.toContain('.diffFileRowSelected .diffFileRowActions');
    expect(css).not.toContain('.diffFileRowFocused .diffFileRowActions');
    expect(css).not.toContain('.diffFileRowSelected .diffFileRowCounts');
    expect(css).not.toContain('.diffFileRowFocused .diffFileRowCounts');
    expect(drawerRule).toContain('position: absolute');
    expect(drawerRule).toContain('width: min(320px, 88%)');
    expect(headerFilesButtonRule).toContain('flex: 0 0 auto');
    expect(headerFilesButtonRule).toContain('height: 26px');
    expect(headerFilesLabelRule).toContain('min-width: 0');
    expect(css).toContain('@container (max-width: 560px)');
    expect(css).toContain('.diffPanelCount');
    expect(css).toContain('display: none');
    expect(css).not.toContain('@container (min-width: 720px)');
    expect(css).toContain('.diffPanelSplitLayout .diffHeaderFilesButton');
    expect(css).toContain('--review-ship-popover-right');
    ...
    expect(fileNavigator).toContain("viewMode() === 'tree' ? 'tree' : 'listbox'");
    expect(fileNavigator).toContain("viewMode() === 'tree' ? 'treeitem' : 'option'");
    expect(fileNavigator).toContain('aria-expanded');
